              ...
              {radiantPlayers.map((player: any) => (
                <div key={player.id} className="flex items-center gap-4 bg-black/40 p-4 rounded-2xl border border-white/5">
                  ...
                  <div className="flex-1 min-w-0">
                    <a href={`/profile/${player.user.id}`} className="font-bold text-lg text-white truncate flex items-center gap-2 hover:text-primary transition-colors cursor-pointer">
                      <img src={getRankFromMMR(player.user.mmr || 1000).iconUrl} alt="Rank" className="w-5 h-5 object-contain" />
                      {player.user.username}
                    </a>
                    <div className="flex gap-2 mt-1">
                      <span className="text-xs px-2 py-0.5 bg-white/5 rounded text-white/50">{getRankFromMMR(player.user.mmr || 1000).name}</span>
